...
if(isset($_POST["btnOK"]))
{
  ...
  $commandText = "select * from students where cName like '%$sUserName%'";
       $result = mysqli_query($link, $commandText);
          $row = mysqli_fetch_assoc($result);
  
  //echo $row['cName'] . "<hr>";
  //echo $row['cPhone'] . "<hr>";
  //echo $_POST["txtPassword"] . "<hr>";
  //echo $_POST["txtUserName"] . "<hr>";
  ...
  else
  {
    ...
    else if($row['cName'] != $_POST["txtUserName"])
    {
       $message = "您的帳號輸入錯誤";
    }
    else
    {
      if($row['cPhone'] != $_POST["txtPassword"])
      {
        $message = "您的密碼輸入錯誤";
      }
      else
